...
=end
  ...
  def smartDamageCategory(attacker,opponent,field=@battle.FE)
    ...
    calcatkmult *= 2 if (attacker.ability== :PUREPOWER && field != :PSYTERRAIN) || attacker.ability== :HUGEPOWER
    ...
    # Lawds Mod 6/19/2024 - Wildfire; Sky triggers Solar Power and Wildfire if Rain, Sand, Hail aren't up. Infernal also activates Wildfire.
    calcspatkmult *= 1.5 if (attacker.ability== :SOLARPOWER || attacker.ability== :WILDFIRE) && ((@battle.pbWeather== :SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || (field == :SKY && ![:RAINDANCE, :HAIL, :SANDSTORM].include?(@battle.pbWeather)) || (field==:INFERNAL && attacker.ability==:WILDFIRE)) && field != :FROZENDIMENSION
    calcspatkmult *= 1.3 if attacker.pbPartner.ability== :BATTERY
    calcspatkmult *= 2 if attacker.ability== :PUREPOWER && field == :PSYTERRAIN
    # end spatk boosts
    ...
    calcdefmult*=1.5 if opponent.ability== :GRASSPELT && (field == :GRASSY || field == :FOREST || @battle.state.effects[:GRASSY] > 0) # Grassy Field
    ...
    calcspdef=opponent.getSpecialStat(attacker.ability==:UNAWARE) if field==:GLITCH
    calcspatk=attacker.getSpecialStat(opponent.ability==:UNAWARE && !(attacker.ability==:MOLDBREAKER && !opponent.hasWorkingItem(:ABILITYSHIELD))) if field==:GLITCH
...
################################################################################
  def pbTypeModifier(type,attacker,opponent,zorovar=false,ai_moldbroken=false,field=@battle.FE) # lawds field parameter for AI
    ...
    if Rejuv && (field == :ELECTERRAIN || @battle.state.effects[:ELECTERRAIN]>0) && attacker.ability== :TERAVOLT # lawds
      ...
      when :SKUNTANK
        if type == :GROUND
          if opponent.pbCanIncreaseStatStage?(PBStats::ATTACK,nil,nil,:SKUNCREST)
            opponent.pbIncreaseStatBasic(PBStats::ATTACK,1,nil,nil,:SKUNCREST)
            @battle.pbCommonAnimation("StatUp",opponent,nil)
  ...
  def pbTypeModMessages(type,attacker,opponent)
    ...
    if @battle.FE == :ROCKY && (opponent.effects[:Substitute]>0 || opponent.stages[PBStats::DEFENSE] > 0) && attacker.ability!=:TRUESHOT # lawds true shot ignores rocky field bullshit
    ...
    if opponent.crested
      ...
      when :DHELMISE
        ...
        typemod = 0 if type == :POISON && attacker.ability!=:CORROSION
      end
    ...
    # Cutting typemod in half                                                               # LAWDS - Rocky Payload lets its boosted moves always hit Flying types for SE regardless of weather
    if @battle.pbWeather==:STRONGWINDS && (opponent.hasType?(:FLYING) && !opponent.effects[:Roost]) && !(attacker.ability== :ROCKYPAYLOAD && (type == :ROCK || (type == :ICE && @battle.FE == :SNOWYMOUNTAIN))) &&
...
################################################################################
  def pbAccuracyCheck(attacker,opponent)
    ...
    if attacker.ability== :VICTORYSTAR || attacker.ability== :KEENEYE # Lawds Mod - Keen Eye boosts own accuracy
      accuracy*=1.1
    end
    ...
    if attacker.ability== :HUSTLE && @basedamage>0 && pbIsPhysical?(pbType(attacker))
        accuracy*= [:BACKALLEY,:CITY].include?(@battle.FE) ? 0.7 : 0.8
    end
...
################################################################################
  def pbCritRate?(attacker,opponent)
    ...
    # LAWDS merciless on infernal
    return 3 if attacker.ability== :MERCILESS && (opponent.status == :POISON || [:CORROSIVE,:CORRPSIVEMIST,:WASTELAND,:MURKWATERSURFACE].include?(@battle.FE) || (opponent.status==:BURN && @battle.FE == :INFERNAL))
    ...
    return 3 if @function==0x218 && (opponent.pbOwnSide.effects[:StealthRock]) # LAWDS - Razor Thread
    c=0    
    c+=attacker.effects[:FocusEnergy]
    c+=1 if !@data.nil? && highCritRate?
    # Lawds Mod - Decidueye Crest
    c+=1 if attacker.crested == :DECIDUEYE
  ...
  def pbCalcDamage(attacker,opponent,options=0, hitnum: 0)
    ...
    case i
      ...
      # Lawds Mod 6/19/2024 - Rampant Growth boosts Grass-type moves 1.5x
      when :RAMPANTGROWTH
        if @type == :GRASS
          basemult*=1.5
        end
      ...
      when :RECKLESS      then basemult*=1.2 if [0xFA,0xFE,0xFD,0x108,0x130].include?(@function)
    ...
    if Rejuv && (@battle.FE==:ELECTERRAIN || @battle.state.effects[:ELECTERRAIN]!=0) && type == :ELECTRIC && attacker.ability== :TERAVOLT
	basemult *= 1.5
    end
    abil = []
    ...
    if attitemworks
      if $cache.items[attacker.item].checkFlag?(:typeboost) == type
        ...
        elsif ($cache.items[attacker.item].checkFlag?(:incense) || (attacker.item==:PUREINCENSE && type==:NORMAL)) && @battle.FE==:PSYTERRAIN && (!attacker.hasType?(type) || type==:NORMAL) && attacker.item!=:WAVEINCENSE
          basemult*=1.25
        end
      else
        case attacker.item
    ...
    basemult*=0.5 if opponent.crested == :PACHIRISU && attacker.hp/attacker.totalhp < 1/2
    ...
    # lawds pure power on PT
    if pbIsSpecial?(type) && !((@battle.FE==:PSYTERRAIN || @battle.state.effects[:PSYTERRAIN]>0) && attacker.ability==:PUREPOWER)
      atk=attacker.spatk
    ...
    # lawds fixed unaware ignoring unaware
    if attacker.ability== :UNAWARE && (opponent.ability!= :UNAWARE || opponent.moldbroken)
       atkstage=attacker.stages[PBStats::ATTACK]+6
       atk=(atk*1.0*stagemul[atkstage]/stagediv[atkstage]).floor
    end
    ...
    case i
      ...
      when :PUREPOWER
        ...
        end                           # LAWDS - Solar Power is activated in Sky if Hail, Sand, Rain aren't up. Wildfire is activated in Infernal.
      when :SOLARPOWER, :WILDFIRE then atkmult*=1.5 if (attacker.pbPartner.crested == :CHERRIM || attacker.crested == :CHERRIM || (@battle.pbWeather== :SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || (@battle.FE == :SKY && ![:RAINDANCE, :HAIL, :SANDSTORM].include?(@battle.pbWeather) && i==:SOLARPOWER) || (@battle.FE == :INFERNAL && i==:WILDFIRE)) && pbIsSpecial?(type) && (@battle.FE != :GLITCH &&  @battle.FE != :FROZENDIMENSION)
    ...
    if @battle.FE != :INDOOR
      ...
      atkmult*=1.2 if @battle.FE==:DARKCRYSTALCAVERN && attacker.ability== :ILLUSION && attacker.effects[:Illusion]!=nil
      abil = []
      if attacker.ability.is_a?(PokeAbility) && attacker.ability.ability!=nil # second conditional catches nils
        if attacker.PULSE3==true || attacker.ability.ability.is_a?(Array)
          for i in attacker.ability.ability
    ...
    if (@battle.pbWeather== :SANDSTORM || @battle.FE == :ROCKY) && opponent.hasType?(:ROCK) && applysandstorm # LAWDS - rocky field spdef boost for rock types
    ...
    case type
      ...
      when :WATER
        if @battle.pbWeather == :RAINDANCE || ((@battle.pbWeather == :SUNNYDAY || attacker.crested == :CHERRIM || attacker.pbPartner.crested == :CHERRIM) && @move == :HYDROSTEAM) # LAWDS - Cherrim Crest
          damage*=1.5
    ...
    # STAB      # LAWDS - spirit's envoy, inverse seed normalize effect
    if (attacker.hasType?(type) && (!attacker.effects[:DesertsMark])|| (attacker.ability== :STEELWORKER && type == :STEEL)  || (attacker.ability== :SOLARIDOL && type == :FIRE) || (attacker.ability== :LUNARIDOL && type == :ICE) || (attacker.ability== :SPIRITSENVOY && type == :GHOST) || (attacker.effects[:seed_normalize] && type == :NORMAL) || typecrest==true)
    ...
    damage*=0.5 if attacker.status== :BURN && pbIsPhysical?(type) && attacker.ability!= :GUTS && attacker.ability!= :QUICKFEET && @move != :FACADE
    # Random Variance
    ...
    finalmult*=1.15 if attacker.crested == :TYPHLOSION && (attacker.hp == attacker.totalhp) && (pbIsSpecial?) # Lawds Mod - Typhlosion Crest rework
    ...
    finalmult*=0.50 if opponent.crested == :VESPIQUEN && opponent.damagestate.typemod>4 # LAWDS vespiquen crest reduces super effective damage by 50% in defense mode
    ...
    # lawds focus band
    totaldamage=(totaldamage*0.9) if opponent.hasWorkingItem(:FOCUSBAND) && totaldamage < opponent.hp
...
################################################################################
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    damage *= 0.15 if hitnum == 1 && attacker.effects[:TyphBond] && pbNumHits(attacker)==1 # LAWDS - nerf to the typhlosion crest second hit damage
    # Lawds Mod 6/19/2024 - Dreadnought
    damage /= 4 if hitnum > 0 && attacker.effects[:DreadBond] && pbNumHits(attacker) > 0
    if opponent.damagestate.typemod!=0 
      pbShowAnimation(@move,attacker,opponent,hitnum,alltargets,showanimation) 
  ...
  def priorityCheck(attacker)
    ...
    pri -= 1 if attacker.ability== :MYCELIUMMIGHT && @basedamage==0 && attacker.effects[:TwoTurnAttack] == 0 # LAWDS - Mycelium Might
